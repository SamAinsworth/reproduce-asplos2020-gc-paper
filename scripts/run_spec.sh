...
do
  ...
  $BASE/scripts/gem5_scripts/run_cfi.sh $BIN "$ARGS" "$IN" "$BOUNDS"
  $BASE/scripts/gem5_scripts/run_counter.sh $BIN "$ARGS" "$IN"
  $BASE/scripts/gem5_scripts/run_rowhammer.sh $BIN "$ARGS" "$IN"
  $BASE/scripts/gem5_scripts/run_shadow.sh $BIN "$ARGS" "$IN"
  cd ../run_base_ref_armdlmalloc.0000
  $BASE/scripts/gem5_scripts/run_sanitizer.sh $BIN "$ARGS" "$IN") &
done
cd $BASE/scripts
